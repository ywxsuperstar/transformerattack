...
from ...cases.models.losses import CausalLoss
...
class GradientLoss(torch.nn.Module):
    ...
    def initialize(self, loss_fn, cfg_impl, local_hyperparams=None):
        ...
        if self.local_hyperparams is None:
            self._grad_fn = self._grad_fn_single_step   # 使用单步的grad???
        else:
    ...
    def _grad_fn_single_step(self, model, candidate, labels):
        ...
        with torch.autocast(candidate.device.type, enabled=self.cfg_impl.mixed_precision):
            print("计算得分：")
            output = model(inputs_embeds=candidate, unk_mask=torch.zeros(1, candidate.size(1), dtype=torch.long).to(candidate.device))
            task_loss = loss_fn(output, labels)
...
# linear
def grad_euclidean(gradient_dummy: List[torch.Tensor], gradient_rec: List[torch.Tensor]):
    objective = gradient_dummy[0].new_zeros(1, )
    for rec, data in zip(gradient_dummy, gradient_rec):
        objective += (rec - data).pow(2).sum()
    return 0.5 * objective
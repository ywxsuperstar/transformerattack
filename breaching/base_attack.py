...
class _BaseAttacker:
    ...
    def prepare_attack(self, server_payload, shared_data):
        ...
        if hasattr(metadata, "mean"):
            ...
            self.dm, self.ds = torch.tensor(0, **self.setup), torch.tensor(1, **self.setup)  # dm:0  ds:1

        # Load server_payload into state:

        # 为什么需要server_payload和shared_data共同才能得到rec_models模型
        rec_models = self._construct_models_from_payload_and_buffers(server_payload, shared_data)
        shared_data = self._cast_shared_data(shared_data)  # 映射到cuda到？（本来也是在cuda上！）
        ...
        # Consider label information    server_payload是parameters  shared_data是gradients??
        if shared_data[0]["metadata"]["labels"] is None:    # label是None的
    ...
    def _postprocess_text_data(self, reconstructed_user_data, models=None):
        ...
        def _max_similarity(recovered_embeddings, true_embeddings):
            recovered_embeddings = recovered_embeddings - recovered_embeddings.mean(dim=-1, keepdim=True)  # 平均归一化处理
            true_embeddings = true_embeddings - true_embeddings.mean(dim=-1, keepdim=True)  # 通过减去每个向量的平均值，将向量的均值调整为零
    ...
    def _construct_models_from_payload_and_buffers(self, server_payload, shared_data):
        ...
        models = []
        for idx, payload in enumerate(server_payload):    # server_payload
            ...
            with torch.no_grad():
                for param, server_state in zip(new_model.parameters(), parameters):
                    param.copy_(server_state.to(**self.setup))   # 将服务器上的模型参数（server_state）复制到客户端的模型参数（param）上
    ...
    def _recover_label_information(self, user_data, server_payload, rec_models):
        ...
        if self.cfg.label_strategy is None:
            ...
            total_min_vals = 0
            for query_id, shared_data in enumerate(user_data):   # 就只有gradients一个键

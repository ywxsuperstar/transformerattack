...
class OptimizationBasedAttacker(_BaseAttacker):
    ...
    def __init__(self, model, loss_fn, cfg_attack, setup=dict(dtype=torch.float, device=torch.device("cpu"))):
        ...
        if objective_fn is None:
            ...
            print(objective_fn(**self.cfg.objective))
        self.regularizers = []
        try:
    ...
    def reconstruct(self, server_payload, shared_data, server_secrets=None, initial_data=None, dryrun=False):
        ...
        rec_models, labels, stats = self.prepare_attack(server_payload, shared_data)   # 得到标签
        # Main reconstruction loop starts here:
        scores = torch.zeros(self.cfg.restarts.num_trials)   #加载测试数量
    ...
    def _run_trial(self, rec_model, shared_data, labels, stats, trial, initial_data=None, dryrun=False):
        ...
        # Initialize candidate reconstruction data   num_data_points假数据个数
            ...
            for iteration in range(self.cfg.optim.max_iterations):
                ...
                with torch.no_grad():
                    ...
                    if self.cfg.optim.boxed:
                        candidate.data = torch.max(torch.min(candidate, (1 - self.dm) / self.ds), -self.dm / self.ds)  # 限制范围在服务器的(-self.dm / self.ds, (1 - self.dm) / self.ds)内
